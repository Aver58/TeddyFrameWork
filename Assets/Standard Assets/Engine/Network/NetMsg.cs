...
public static class NetMsg
{
    ...
    public static void SendMsg(NetMsgData data)
    {
        ...
        Log.DebugFormat("[Client]client send: ID:{0},Data:{1}", data.ID, data.Data);
    }
    ...
    public static void HandleMsg(byte[] buffer)
    {
        ...
        {
            Log.DebugFormat("[Server]收到 ：protoID：{0}，data：{1}", protoID, data.Data);
            if (callback!=null)
            {
                callback(data.Data);
            }
        ...
        {
            Log.DebugFormat("[Server]收到未监听的服务器消息：protoID：{0}，data：{1}", protoID, data.Data);
        }
    }
