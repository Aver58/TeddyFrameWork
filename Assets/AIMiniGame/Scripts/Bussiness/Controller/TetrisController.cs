...
namespace AIMiniGame.Scripts.TetrisGame
{
    public class TetrisController : ControllerBase
    {
        ...
        private MonoBehaviour _monoBehaviour; // 用于启动协程
        private Coroutine _gameLoopCoroutine;

        public TetrisController(MonoBehaviour monoBehaviour)
        {
        ...
        public void StartGame()
        {
            ...
            if (_gameLoopCoroutine != null)
                _monoBehaviour.StopCoroutine(_gameLoopCoroutine);
                
            _gameLoopCoroutine = _monoBehaviour.StartCoroutine(GameLoop());
        }

        private IEnumerator GameLoop()
        {
            while (!TetrisModel.IsGameOver)
            {
        ...
        private bool MoveTetromino(int offsetX, int offsetY)
        {
            Vector2Int newPosition = new Vector2Int(
                TetrisModel.CurrentPosition.x + offsetX,
                TetrisModel.CurrentPosition.y + offsetY
            );
