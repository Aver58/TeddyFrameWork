...
public static class StartExternalProcess {
    ...
    public static uint Start(string path, string dir, bool hidden = false) {
        ...
        if (!CreateProcessW(null, path, IntPtr.Zero, IntPtr.Zero,
                false, flags, IntPtr.Zero, dir, ref startupinfo, ref processinfo)) {
    ...
    public static int KillProcess(uint pid) {
        IntPtr handle = OpenProcess(ProcessAccessRights.SYNCHRONIZE, false, pid);
