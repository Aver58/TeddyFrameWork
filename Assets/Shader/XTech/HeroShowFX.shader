Shader "XTech/HeroShowFX"
{
	Properties
	{
		...
		_BasicValue("BasicValue",float) = 0.5
		_FXColor("_FXColor",color) = (1,1,1,1)
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				o.uv1 = TRANSFORM_TEX(v.uv, _FXTex).xy;
				o.worldNormal = UnityObjectToWorldNormal(v.normal);
				o.worldPos = mul(UNITY_MATRIX_M, v.vertex);
				o.worldPos.w = 1 - v.vertex.x;
				o.uv.zw = mul((float3x3)UNITY_MATRIX_IT_MV, v.normal).xy * 0.5 + float2(0.5, 0.5);
				o.time = _Time;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 FX = tex2D(_FXTex, i.uv1 + float2(i.time.x, 0))  ;
				fixed4 FXPower = float4(FX.rgb * _FXColor.rgb,1);

				fixed4 lastColor = fixed4(col.rgb + specular, 1) * FXPower * _FXColor.a + fixed4(col.rgb + specular , 1) *_BasicValue ;
