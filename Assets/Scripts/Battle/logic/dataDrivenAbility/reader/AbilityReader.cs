 ...
 */
                ...
                for (int i = 0; i < valueAdditionParamsConfig.Count; i++) {
                    ...
                    if (valueSourceParamsConfig != null) {
                        if (valueSourceParamsConfig.IsArray) {
                            int sourceValueCoefficient =
                                valueSourceParamsConfig.Count >= 1 ? (int) valueSourceParamsConfig[0] : 0;
                            int sourceValueCoefficientGrow = valueSourceParamsConfig.Count >= 2
                                ? (int) valueSourceParamsConfig[1]
                                : 0;
                            int limitBasicValue = valueSourceParamsConfig.Count >= 3
                                ? (int) valueSourceParamsConfig[2]
                                : 0;
                            int limitBasicValueGrow = valueSourceParamsConfig.Count >= 4
                                ? (int) valueSourceParamsConfig[3]
                                : 0;
                            abilityValueSource.AddAdditionValueParams(valueSourceType,
                                sourceValueCoefficient, sourceValueCoefficientGrow, limitBasicValue,
                                limitBasicValueGrow);
                        }
                        else
                            BattleLog.LogError("技能[%s]中的[ValueSource]的[ValueSourceParams]的配置必须是数组类型",
                                abilityData.configFileName);
                    }
                }
            }
            else
    ...
    private static AbilityTarget ParseAbilityRange(JsonData json, AbilityData abilityData) {
        ...
        if ((abilityBehavior & AbilityBehavior.ABILITY_BEHAVIOR_SECTOR_AOE) != 0) {
            ...
            if (sectorJsonData == null) {
                BattleLog.LogError("技能[{0}]行为是ABILITY_BEHAVIOR_SECTOR_AOE，未找到AbilityAoeSector配置",
                    abilityData.configFileName);
                return null;
            }
        ...
        if ((abilityBehavior & AbilityBehavior.ABILITY_BEHAVIOR_RADIUS_AOE) != 0) {
            ...
            if (radiusJson == null) {
                BattleLog.LogError("技能[{0}]行为是ABILITY_BEHAVIOR_RADIUS_AOE，未找到AbilityAoeRadius配置",
                    abilityData.configFileName);
                return null;
            }
    ...
    private static void ParseModifierAura(JsonData json, AbilityData abilityData, ModifierData modifier) {
        ...
        if (modifier.Aura != null) {
            modifier.Aura_Teams = GetEnumValue<AbilityUnitTargetTeam>(json, "Aura_Teams");
            if (modifier.Aura_Teams == default) {
                BattleLog.LogError("技能[{0}]的Modifier[{1}]中配置了光环类型，但是Aura_Teams未找到或者配置有错误", abilityData.configFileName,
                    modifier.Name);
                return;
            }

            modifier.Aura_Types = GetEnumValue<AbilityUnitTargetType>(json, "Aura_Types");
            if (modifier.Aura_Teams == default) {
                BattleLog.LogError("技能[{0}]的Modifier[{1}]中配置了光环类型，但是Aura_Types未找到或者配置有错误", abilityData.configFileName,
                    modifier.Name);
                return;
            }
            ...
            if (modifier.Aura_Teams == default) {
                BattleLog.LogError("技能[{0}]的Modifier[{1}]中配置了光环类型，但是Aura_Radius未配置", abilityData.configFileName,
                    modifier.Name);
                return;
            }
        }
            ...
            foreach (var propertyType in propertyDataConfig.Keys) {
                ...
                if (propertyEnum == default) {
                    ...
                    if (propertyValueConfig != null) {
                        ...
                        BattleLog.LogError("技能[{0}]中的ModifierProperty类型[{1}]不是数组配置或者ValueSource类型",
                            abilityData.configFileName, propertyType);
                    }
                }
            }
    ...
    private static AbilityData CreateAbility(string path) {
        ...
        abilityData.aiTargetCondition =
            GetEnumValue<AbilityUnitAITargetCondition>(jsonData, "AbilityUnitAiTargetCondition");
