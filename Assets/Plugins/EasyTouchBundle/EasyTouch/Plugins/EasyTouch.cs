...
//	}
		...
		if ( fingers[fingerIndex].gesture==GestureType.None){
			...
			if (autoSelect){
				if (GetPickedGameObject(fingers[fingerIndex])){
					...
					fingers[fingerIndex].hitPoint = pickedObject.hitPoint;
				}
			}
		...
		if ( twoFinger.currentGesture==GestureType.None){
			...
			if (autoSelect){
				if (GetTwoFingerPickedObject()){
					...
					twoFinger.hitPoint = pickedObject.hitPoint;
				}
				else{
	...
	private void  CreateGesture2Finger(EventName message,Vector2 startPosition,Vector2 position,Vector2 deltaPosition,
		...
		gesture.hitPoint = twoFinger.hitPoint;
...
//				}
//			}
		    if (IsScreenPositionOverUI(fingers[twoFinger.finger0].position)|| IsScreenPositionOverUI(fingers[twoFinger.finger1].position))
		    {
				...
				if (touchCameras[i].camera!=null && touchCameras[i].camera.enabled){
					...
					if (enable2D){
						...
						if (Physics2D.GetRayIntersectionNonAlloc( ray,hit2D,float.PositiveInfinity,mask2d)>0){
							...
							pickedObject.hitPoint = hit2D[0].point;
							return true;
						}
					}
			        ...
			        if( Physics.Raycast( ray, out hit,float.MaxValue,mask ) ){
						...
						pickedObject.hitPoint = hit.point;
			            return true;
					}
				}
			}
	...
	public static void SetEnabled( bool enable){
        ...
        EasyTouch.instance.enabled = enable;
        //禁用的时候清理掉finger数据
        if (!enable){
